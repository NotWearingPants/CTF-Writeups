...
It printed `0, 5, 2, 7, 4, 1, 6, 3`. So it turns out I was right about it being a 3-bit value, but it looks like it overflows and adding 5 to it each time actually goes through the indices 0..7 without repeats.
This means that if I tried to follow the bits myself, I wouldn't have notices this, and would have gotten the password shuffled and would probably debug my script for hours without finding the problem.
